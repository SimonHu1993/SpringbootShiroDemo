layui.define(['element'], function(exports){
   var  element = layui.element,
    ...
    LarryTab.prototype.set = function(options){
          var _this = this;
          $.extend(true, _this.tabConfig, options);
          return _this;
    };
    /**
     * [init 对象初始化]
	    ...
	    if(tabIndex === -1){
			...
			//监听关闭事件
			    ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
			    	...
			    	element.tabDelete(ELEM.tabFilter,_this.getLayId(data.title)).init();
			    });
		    };
		    //切换到当前打开的选项卡
			var index_ = ELEM.titleBox.find('li');
			// console.log(_this.getLayId(data.title))
		    // element.tabChange(ELEM.tabFilter, ELEM.titleBox.find('li').length - 1);
		    }else {
			    // element.tabChange(ELEM.tabFilter, tabIndex);
		    }
		element.tabChange(ELEM.tabFilter,_this.getLayId(data.title)).init();
        };

	//通过title获取lay-id
	LarryTab.prototype.getLayId = function(title){
		$(".layui-tab-title li").each(function(){
			if($(this).find("em").text() == title){
